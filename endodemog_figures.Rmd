...
variance_histogram <- function(df){
  ...
  spp <- ggplot() +
    ...
    ylim(0,2500) +
    labs(x = expression(σ[year]), y = "Posterior density") + theme_classic()
  return(spp)
}
# survival
...
means_histogram <- function(df){
  require(ggplot2)
    df$mean_eminus_y1 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,1]` 
    df$mean_eminus_y2 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,2]` 
    df$mean_eminus_y3 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,3]` 
    df$mean_eminus_y4 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,4]` 
    df$mean_eminus_y5 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,5]` 
    df$mean_eminus_y6 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,6]` 
    df$mean_eminus_y7 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,7]` 
    df$mean_eminus_y8 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,8]` 
    df$mean_eminus_y9 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,9]` 
    df$mean_eminus_y10 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,10]` 
    df$mean_eminus_y11 <- df$`beta[1]` + df$`beta[2]` + df$`tau_year[1,11]` 

    df$mean_eplus_y1 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,1]` 
    df$mean_eplus_y2 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,2]` 
    df$mean_eplus_y3 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,3]` 
    df$mean_eplus_y4 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,4]` 
    df$mean_eplus_y5 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,5]` 
    df$mean_eplus_y6 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,6]` 
    df$mean_eplus_y7 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,7]` 
    df$mean_eplus_y8 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,8]` 
    df$mean_eplus_y9 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,9]` 
    df$mean_eplus_y10 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,10]` 
    df$mean_eplus_y11 <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]` + df$`tau_year[2,11]` 
    
    df$mean_eminus_sum <- df$mean_eminus_y1 + df$mean_eminus_y2 + df$mean_eminus_y3 + df$mean_eminus_y4 + df$mean_eminus_y5 + df$mean_eminus_y6 + df$mean_eminus_y7 + df$mean_eminus_y8 + df$mean_eminus_y9 + df$mean_eminus_y10 + df$mean_eminus_y11
    
    df$mean_eplus_sum <- df$mean_eplus_y1 + df$mean_eplus_y2 + df$mean_eplus_y3 + df$mean_eplus_y4 + df$mean_eplus_y5 + df$mean_eplus_y6 + df$mean_eplus_y7 + df$mean_eplus_y8 + df$mean_eplus_y9 + df$mean_eplus_y10 + df$mean_eplus_y11

    df$diff_mean_sums <- df$mean_eminus_sum - df$mean_eplus_sum
    
    df$mu_eminus <- df$`beta[1]` + df$`beta[2]`
    df$mu_eplus <- df$`beta[1]` + df$`beta[2]` + df$`beta[3]`
    df$diff_mu <- df$mu_eminus - df$mu_eplus
    
  spp <- ggplot() +
    # geom_histogram(data = df, aes(mean_eminus_sum), fill = "#ff7f00", alpha = .4, bins = 200) +
    # geom_histogram(data = df, aes(mean_eplus_sum), fill = "#6a3d9a", alpha = .4, bins = 200) +
    # geom_histogram(data = df, aes(diff_mean), fill = "#4d9158", alpha = .6, bins = 200) +
    # geom_histogram(data = df, aes(mu_eminus), fill = "#ff7f00", alpha = .4, bins = 200) +
    # geom_histogram(data = df, aes(mu_eplus), fill = "#6a3d9a", alpha = .4, bins = 200) +
    geom_histogram(data = df, aes(diff_mu), fill = "#4d9158", alpha = .6, bins = 200) +
    # geom_vline(xintercept = 0) +
    geom_vline(xintercept = mean(df$diff_mu), col = "#4d9158") +
    labs(x = expression(mu[year]), y = "Posterior density") + theme_classic()
  return(spp)
}
# survival
...
surv_data_bins <- function(df_raw){
  ...
  surv_bin_minus <- df_raw %>% 
    select(surv_t1, logsize_t, year_t, endo_01) %>%
    filter(endo_01 == 0) %>%
    rename(Endo = endo_01) %>% 
  ...
  surv_bin_mean <- df_raw %>% 
    select(surv_t1, logsize_t, endo_01) %>%
...
surv_fits <- function(df_raw, df_samples){
  ...
  for(i in 1:10000){
    meanminussamps[,i] <- invlogit(sample(df_samples$'beta[1]', size = 1) + sample(df_samples$'beta[2]', size = 1)*xdummy + sample(df_samples$'beta[3]', size = 1)*0 + sample(df_samples$'beta[4]', size = 1)*mean(df_raw$origin_01) + sample(df_samples$'beta[5]', size = 1)*xdummy*0)
}
 ...
 meanminus500samps <- as.data.frame(meanminussamps[,sample(ncol(meanminussamps), 500)]) %>% 
  ...
  for(i in 1:10000){
    meanplussamps[,i] <- invlogit(sample(df_samples$'beta[1]', size = 1) + sample(df_samples$'beta[2]', size = 1)*xdummy + sample(df_samples$'beta[3]', size = 1)*1 + sample(df_samples$'beta[4]', size = 1)*mean(df_raw$origin_01) + sample(df_samples$'beta[5]', size = 1)*xdummy*1)
}
