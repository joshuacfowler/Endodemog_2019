...
mcmc_dens_overlay(flow_fit,pars = c("mu_betaendo","mu_sigmaendo"))
mcmc_dens_overlay(flow_fit,pars = c("mu_betaendo","mu_sigmaendo"))
mcmc_rhat(rhat(flow_fit,pars = c("mu_betaendo","mu_sigmaendo")))
mcmc_dens_overlay(flow_fit,pars = c("betaendo"))

predF <- rstan::extract(flow_fit, pars = c("p"))$p
n_post_draws <- 100
post_draws <- sample.int(dim(predF)[1], n_post_draws)
y_f_sim <- matrix(NA,n_post_draws,length(flow_dat$y))
for(i in 1:n_post_draws){
  y_f_sim[i,] <- rbinom(n=length(flow_dat$y), size=1, prob = invlogit(predF[post_draws[i],]))
}
ppc_dens_overlay(flow_dat$y, y_f_sim)

mcmc_trace(flow_fit,par=c("beta0[1]"))
mcmc_trace(flow_fit,par=c("betasize[1]"))
mcmc_trace(flow_fit,par=c("betaendo"))
...
par(mfrow=c(2,1))
plot(rep(0,7),1:7,type="n",xlim=c(-4,4))
abline(v=0,lty=2,col="gray")
points(betaendo_flow_mean,1:7,cex=2,pch=16)
arrows(betaendo_flow_quant[1,],1:7,betaendo_flow_quant[4,],1:7,length=0,lwd=2)
arrows(betaendo_flow_quant[2,],1:7,betaendo_flow_quant[3,],1:7,length=0,lwd=4)

plot(rep(0,7),1:7,type="n",xlim=c(-4,4))
abline(v=0,lty=2,col="gray")
points(sigmaendo_flow_mean,1:7,cex=2,pch=16)
arrows(sigmaendo_flow_quant[1,],1:7,sigmaendo_flow_quant[4,],1:7,length=0,lwd=2)
arrows(sigmaendo_flow_quant[2,],1:7,sigmaendo_flow_quant[3,],1:7,length=0,lwd=4)