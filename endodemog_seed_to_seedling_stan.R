...
# I'm gonna merge the seed estimates with the fert data which is only the plants that are flowering to get estimates.

post_seedAGPE <- extract(sm_seedAGPE)
post_spikeAGPE <- extract(sm_spikeAGPE)
AGPE_surv_data$fixedeffect <- sample(post_spikeAGPE$beta[,1], size = length(AGPE_surv_data$FLW_STAT_T)) + 
                          sample(post_spikeAGPE$beta[,2], size = length(AGPE_surv_data$FLW_STAT_T))*AGPE_surv_data$logsize_t +
                          sample(post_spikeAGPE$beta[,3], size = length(AGPE_surv_data$FLW_STAT_T))*AGPE_surv_data$endo_01 +
                          sample(post_spikeAGPE$beta[,4], size = length(AGPE_surv_data$FLW_STAT_T))*AGPE_surv_data$origin_01 +
                          sample(post_spikeAGPE$beta[,5], size = length(AGPE_surv_data$FLW_STAT_T))*AGPE_surv_data$logsize_t*AGPE_surv_data$endo_01
AGPE_surv_data$plot_effect <- ifelse(AGPE_surv_data$plot_index == 1, sample(post_spikeAGPE$tau_plot[,1], size = length(AGPE_surv_data$FLW_STAT_T)), ifelse(AGPE_surv_data$plot_index == 2, sample(post_spikeAGPE$tau_plot[,2], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 3, sample(post_spikeAGPE$tau_plot[,3], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 4, sample(post_spikeAGPE$tau_plot[,4], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 5, sample(post_spikeAGPE$tau_plot[,5], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 6, sample(post_spikeAGPE$tau_plot[,6], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 7, sample(post_spikeAGPE$tau_plot[,7], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 8, sample(post_spikeAGPE$tau_plot[,8], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 9, sample(post_spikeAGPE$tau_plot[,9], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$plot_index == 10, sample(post_spikeAGPE$tau_plot[,10], size = length(AGPE_surv_data$FLW_STAT_T)), NA))))))))))
AGPE_surv_data$yearendo_effect <- ifelse(AGPE_surv_data$year_t_index == 1 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,1], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 1 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,1], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 2 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,2], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 2 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,2], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 3 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,3], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 3 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,3], size = length(AGPE_surv_data$FLW_STAT_T)) , ifelse(AGPE_surv_data$year_t_index == 4 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,4], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 4 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,4], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 5 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,5], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 5 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,5], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 6 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,6], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 6 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,6], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 7 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,7], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 7 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,7], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 8 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,8], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 8 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,8], size = length(AGPE_surv_data$FLW_STAT_T)) ,ifelse(AGPE_surv_data$year_t_index == 9 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,9], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 9 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,9], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 10 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,10], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 10 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,10], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 11 & AGPE_surv_data$endo_index == 1, sample(post_spikeAGPE$tau_year[,1,11], size = length(AGPE_surv_data$FLW_STAT_T)),ifelse(AGPE_surv_data$year_t_index == 11 & AGPE_surv_data$endo_index == 2, sample(post_spikeAGPE$tau_year[,2,11], size = length(AGPE_surv_data$FLW_STAT_T)),NA))))))))))))))))))))))
AGPE_surv_data$seedperspike_mean <- sample(post_seedAGPE$mu_seed, size = length(AGPE_surv_data$FLW_STAT_T))

AGPE_surv_data$linpred <-  AGPE_surv_data$fixedeffect+AGPE_surv_data$plot_effect +AGPE_surv_data$yearendo_effect
AGPE_surv_data$spikeperinf_prob = exp(AGPE_surv_data$linpred)

AGPE_surv_data$spikeperinf_pred <- rztnnbinom(n = nrow(AGPE_surv_data), size = mean(post_spikeAGPE$phi), mu = AGPE_surv_data$spikeperinf_prob)
  

AGPE_s_to_s_data <- AGPE_surv_data %>% 
  mutate(seed_est = as.integer(FLW_STAT_T*FLW_COUNT_T*spikeperinf_pred*seedperspike_mean)) %>% 
  group_by(species, plot_index, plot_fixed, year_t, year_t_index, year_t1, year_t1_index, endo_01, endo_index) %>% 
  summarize(tot_seed_t = sum(seed_est, na.rm = TRUE),
            tot_recruit_t1 = length((origin_01 == "1" & year_t == birth)),
            samplesize = n())


table(AGPE_s_to_s_data$plot_index, AGPE_s_to_s_data$year_t_index)


AGPE_datasss <- AGPE_surv_data %>% 
  filter(year_t == birth)
