...
\begin{itemize}
\item{Obtain herbarium seeds to test by Feb. 13th-ish.}
\item{Build up list of herbaria to contact and of specific specimens to look for, so that after our test seeds, we can keep moving forward, and maybe visit as many local herbaria during the semester as possible. Then this Summer, visit some that are farther afield.}
\item{Build growth model by Feb. 13.}
\item{Build seed production model by Feb. 20}
...
\item{Have all parts of the stochastic population model by end of semester (week of May 1st), at least working.}
