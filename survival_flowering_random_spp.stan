...
model {
    ...
    for(s in 1:nSpp){
      beta0[s] ~ normal(mu_beta0,sigma_beta0);
      betasize[s] ~ normal(mu_betasize,sigma_betasize);
      betaendo[s] ~ normal(mu_betaendo,sigma_betaendo);
      sigma0[s] ~ normal(mu_sigma0,sigma_mu_sigma0);
      sigmaendo[s] ~ normal(mu_sigmaendo,sigma_mu_sigmaendo);
      for(d in 1:nEndo){
        for(t in 1:nYear){
